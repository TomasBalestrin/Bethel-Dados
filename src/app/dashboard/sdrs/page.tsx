...
import { useUsers, useSdrMetrics, useGoals } from "@/hooks/use-data";
...
export default function SdrsPage() {
  ...
  const isLoading = loadingUsers || loadingSdr || loadingGoals;
  ...
  const sdrUsers = users.filter((u) => u.team_id === "t1");

  const totalCalls = sdrMetrics.reduce((s, m) => s + m.calls_made, 0);
  const totalMeetings = sdrMetrics.reduce((s, m) => s + m.meetings_scheduled, 0);
  const totalQualified = sdrMetrics.reduce((s, m) => s + m.qualified_leads, 0);
  const totalEmails = sdrMetrics.reduce((s, m) => s + m.emails_sent, 0);
  const avgConversion = sdrMetrics.length > 0
    ? sdrMetrics.reduce((s, m) => s + m.conversion_rate, 0) / sdrMetrics.length
    : 0;
  ...
  const weeklyData = Array.from({ length: 7 }, (_, i) => {
    ...
    return {
      ...
      value: sdrMetrics.filter((m) => m.date === dateStr).reduce((s, m) => s + m.calls_made, 0),
      value2: sdrMetrics.filter((m) => m.date === dateStr).reduce((s, m) => s + m.meetings_scheduled, 0),
    };
  });
  ...
  const sdrGoals = goals.filter((g) => g.team_id === "t1");
    ...
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
          ...
          <MetricCard title="Follow-ups" value={sdrMetrics.reduce((s, m) => s + m.follow_ups, 0)} icon={Phone} compact />
        </div>
      </div>
